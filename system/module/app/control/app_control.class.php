...
class app_control extends init_control
{
	public function _initialize() {
		...
		$this->app_service = $this->load->service('app/app');
	...
	public function app_develop(){
		if((int)$_GET['id'] > 0){
			...
			$vars = $this->load->table('app/appvar')->where(array('appid' => $_GET['id']))->order('displayorder ASC,id ASC')->select();
		}
	...
	public function setting(){
		...
		$plugin['identifier'] = str_replace('plugin.', '', $plugin['identifier']);
		$vars = model('app/appvar')->where(array('appid' => $_GET['id']))->order("displayorder ASC, id ASC")->select();
