<?php include template('header','admin');?>
		<div class="fixed-nav layout">
			<ul>
				...
				<li><a <?php if($_GET['status'] == 1 || !isset($_GET['status'])){?>class="current"<?php }?> href="<?php echo url('app/app/module_index', array('status' => 1))?>">已启用模块</a></li>
				<li><a <?php if($_GET['status'] == 0 && isset($_GET['status'])){?>class="current"<?php }?>href="<?php echo url('app/app/module_index', array('status' => 0))?>">未启用模块</a></li>
				<li><a <?php if($_GET['status'] == -1){?>class="current"<?php }?>href="<?php echo url('app/app/module_index', array('status' => -1))?>">未安装模块</a></li>
			</ul>
				...
				<div class="tr" data-id="<?php echo $module['branch_id']?>">
					...
					<div class="td">
						<span class="td-con double-row">
							<a href="<?php echo url('app/app/available',array('identifier' => $identifier,'type' => 'module'))?>"><?php if($_GET['status'] == 1 || !isset($_GET['status'])){?>禁用<?php }?><?php if($_GET['status'] == 0 && isset($_GET['status'])){?>开启<?php }?></a> <?php if($_GET['status'] != -1){?> | <?php }?>
							<?php if($_GET['status'] == -1){?>
							<a href="<?php echo url('app/app/install',array('identifier' => $identifier,'type' => 'module'))?>">安装</a>
							<?php }else{?>
							<a href="<?php echo url('app/app/upgrade',array('identifier' => $identifier,'type' => 'module'))?>">更新</a>
							<?php }?>
							<?php if($module['is_system'] != 1){?>
							| <a data-confirm="卸载将清空该模块的所有配置信息，是否确认卸载？" href="<?php echo url('app/app/uninstall',array('identifier' => $identifier,'type' => 'module'))?>">卸载</a>
							<?php }?>
						</span>
					</div>
				</div>
		...
		<script>
			...
			$(function(){
				$.get('<?php echo url("app/app/ajax_upgrade")?>',{},function(ret){
