...
class app_service extends service {
	...
	public function __construct() {
        ...
        $this->app_db = $this->load->table('app/app');
        $this->appvar_db = $this->load->table('app/appvar');
			...
			foreach($xmldata['menu'] as $module) {
				if(is_numeric($module['type'])) {
					$nodes[] = array(
						...
						'url' => !$module['module'] ? $this->get_api_url().'/index.php?m=app&c=app&a=module&mod='.$module['name'] : '',
						'appid' => $appid,
					);
					if($module['setting'] == 1){
						$data = array();
						$data['id'] = $appid;
						$data['url'] = $module['module'] ? $this->get_api_url().'/index.php?m='.$module['module'].'&c='.$module['control'].'&a='.$module['action'] : $this->get_api_url().'/index.php?m=app&c=app&a=module&mod='.$module['name'];
	...
	private function get_pluginvar($appid) {
		$sqlmap = array();
		$sqlmap['appid'] = $appid;
		return $this->load->table('app/appvar')->where($sqlmap)->select();
	}
	/**
